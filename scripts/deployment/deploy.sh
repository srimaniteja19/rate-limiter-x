...
# Deployment Script for Rate Limiter X
...
Deploy Rate Limiter X to Kubernetes
...
verify_deployment() {
    ...
    # Check pod status
    local rate_limiter_pods=$(kubectl get pods -n "$NAMESPACE" -l app.kubernetes.io/name=rate-limiter-x --no-headers | wc -l)
    ...
    local rate_limiter_pod=$(kubectl get pods -n "$NAMESPACE" -l app.kubernetes.io/name=rate-limiter-x -o jsonpath='{.items[0].metadata.name}')
...
show_status() {
    ...
    echo "===================="
    kubectl get all -n "$NAMESPACE" -l app.kubernetes.io/part-of=rate-limiter-x
    echo
    log "Ingress Status:"
