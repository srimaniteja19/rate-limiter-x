# Operations Runbook - Rate Limiter X

This runbook provides step-by-step procedures for operating and troubleshooting the Rate Limiter X service in production.
...
### Service Overview
- **Application**: Rate Limiter X
- **Purpose**: Token bucket based rate limiting with Redis backend
- **Repository**: https://github.com/srimaniteja19/rate-limiter-x
...
```bash
# Check pod status
kubectl get pods -n rate-limiter -l app.kubernetes.io/name=rate-limiter-x

# Check pod logs
kubectl logs -n rate-limiter -l app.kubernetes.io/name=rate-limiter-x --tail=100
...
```bash
# Check error logs
kubectl logs -n rate-limiter -l app.kubernetes.io/name=rate-limiter-x | grep ERROR

# Check Redis connectivity
kubectl exec -n rate-limiter redis-pod -- redis-cli ping

# Check rate limiting violations
kubectl logs -n rate-limiter -l app.kubernetes.io/name=rate-limiter-x | grep "Rate limit VIOLATED"
```
...
kubectl logs -n rate-limiter -l app.kubernetes.io/name=rate-limiter-x | grep "VIOLATED" | awk '{print $NF}' | sort | uniq -c | sort -nr | head -10
