...
This guide explains how to run the Rate Limiter X application using Docker.
...
docker build -t rate-limiter-x:latest .
...
4. **Docker build fails with network errors**
   ...
   docker build --network=host -t rate-limiter-x:latest .
...
# Volume location
docker volume inspect rate-limiter-x_redis-data
```
