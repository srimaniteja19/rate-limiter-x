...
The dashboard connects to a Rate Limiter X backend running on **http://localhost:8080** with the following features:
...
- **Returns**: Real performance data from Rate Limiter X
...
See [GitHub Issue #XXX](https://github.com/srimaniteja19/rate-limiter-x/issues) for implementation tracking.
...
- A Rate Limiter X backend running on `localhost:8080`
...
```bash
# From project root
cd /Users/upp/Development/workspaces/open-source/rate-limiter-x
...
cd rate-limiter-x/examples/web-dashboard
...
The dashboard integrates with a Rate Limiter X backend. See [API_INTEGRATION.md](./API_INTEGRATION.md) for:
